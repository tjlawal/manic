	...
	namespace ResourceManager {
		namespace Parser {
			...
			struct Lexer {
				...
				: input(_input), current_char(_current_char), current_position(_current_pos), future_position(_future_pos) {}
			};

			// Main parsing functions
			internal void* rm_parse_data_from_file(Arena* arena, string8 data);
